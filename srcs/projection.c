...
void	draw_wall(t_data *d, double angle, int x, double l, t_tex tex)
{
	double	di;
	double	h;
	double	y;
...
t_tex	choise_tex(t_casting c, t_data *d, double xy[2])
{
	int	t;
	...
	{
		if (!(fabs(c.lc[0]) < c.ray_step)) /* left edge of tile */
				t = (t + 2) % 4;
		d->tex[t].x = (int)((c.lc[1] / c.sz[1]) * d->tex->w);
	}
	else
	{
		if (fabs(c.lc[1]) < c.ray_step) /* top edge */
       t = (t + 1) % 4;
    else
        t = (t + 3) % 4;
...
void draw_projection(t_data *d, int color)
{
	...
	while (++x < d->w)
	{
		...
		draw_wall(d, c.angle, x, c.pas, choise_tex(c, d, xy));
	}
}
